  ...
  (function ($) {
    ...
    function hasScrollbar($elem, axis) {
      ...
      if (query.target.isWindow) {
        ...
        elem.nodeType === 1 &&
        elem.tagName.toLowerCase() === "iframe"
      ) {
      ...
      if (query.target.isWindow || query.target.isBody) {
        context.window =
          context.document.defaultView || context.document.parentWindow;
      ...
      if (query.target.isWindow) {
        ...
        if (query.getHorizontal)
          result.horizontal =
            bodyProps.overflowScrollX ||
            (bodyProps.overflowAutoX &&
              context.body.clientHeight < $body.height());
        if (query.getVertical)
          result.vertical =
            bodyProps.overflowScrollY ||
            (bodyProps.overflowAutoY &&
              context.body.clientWidth < $body.width());
      } else {
        ...
        elemProps = getCss(elem, ["overflow", "overflowX", "overflowY"], {
          toLowerCase: true,
        });
        ...
        result.horizontal =
          scrollWidth > 0 &&
          (elemProps.overflowScrollX ||
            (elemProps.overflowAutoX &&
              (innerWidth = $elem.innerWidth()) < scrollWidth));
        result.vertical =
          scrollHeight > 0 &&
          (elemProps.overflowScrollY ||
            (elemProps.overflowAutoY &&
              (innerHeight = $elem.innerHeight()) < scrollHeight));
        ...
        result.vertical =
          ...
          (result.horizontal &&
            elemProps.overflowAutoY &&
            (innerHeight !== undefined ? innerHeight : $elem.innerHeight()) -
              $.scrollbarWidth() <
              scrollHeight);
        result.horizontal =
          result.horizontal ||
          (result.vertical &&
            elemProps.overflowAutoX &&
            (innerWidth !== undefined ? innerWidth : $elem.innerWidth()) -
              $.scrollbarWidth() <
              scrollWidth);
      }

      return query.getBoth
        ? result
        : query.getHorizontal
          ? result.horizontal
          : result.vertical;
    ...
    function effectiveScrollbarWith($elem, axis) {
      ...
      if (globalWidth === 0)
        return queryBoth ? { horizontal: 0, vertical: 0 } : 0;
      ...
      if (queryHorizontal)
        horizontal = elemHasScrollbar.horizontal ? globalWidth : 0;
    ...
    function ownerWindow($elem) {
      var elem = $elem[0],
        ownerDocument =
          elem && (elem.nodeType === 9 ? elem : elem.ownerDocument);
    ...
    function _prepareConfig($elem, container, opts) {
      ...
      if (opts.tolerance !== undefined) {
        config.toleranceType =
          isNumber(opts.tolerance) || opts.tolerance.slice(-3) === "px"
            ? "add"
            : "multiply";
        config.tolerance =
          config.toleranceType === "add"
            ? parseFloat(opts.tolerance)
            : parseFloat(opts.tolerance) / 100 + 1;
      } else {
        config.tolerance = 0;
        config.toleranceType = "add";
      }
    ...
    function _isInView(elem, config) {
      ...
      if (elem === container)
        throw new Error("Invalid container: is the same as the element");
      ...
      if (
        config.excludeHidden &&
        !(elem.offsetWidth > 0 && elem.offsetHeight > 0)
      )
        return false;

      if (config.useHorizontal)
        containerWidth = getNetContainerWidth(
          $container,
          config.containerIsWindow,
          cache,
        );
      if (config.useVertical)
        containerHeight = getNetContainerHeight(
          $container,
          config.containerIsWindow,
          cache,
        );
      ...
      rect = config.borderBox
        ...
        : getContentRect(elem);
      if (!config.containerIsWindow)
        rect = getRelativeRect(rect, $container, cache);
    ...
    function getPaddingRectRoot(elem) {
      var rect = elem.getBoundingClientRect(),
        props = getCss(elem, ["borderTopWidth", "borderLeftWidth"], {
          toFloat: true,
        });
    ...
    function _windowHasScrollbar(query, context) {
      ...
      if (query.getHorizontal && scrollbarWidth > 0) {
        ...
        if (windowInnerHeight > 10) {
          result.horizontal =
            windowInnerHeight > context.documentElement.clientHeight;
          doneX = true;
        }
      }
      ...
      if (query.getVertical && scrollbarWidth > 0) {
        ...
        if (windowInnerWidth > 10) {
          result.vertical =
            windowInnerWidth > context.documentElement.clientWidth;
          doneY = true;
        }
      }
      ...
      if (!doneX || !doneY) {
        ...
        if (!doneX && windowProps.overflowScrollX)
          result.horizontal = doneX = true;
        if (!doneY && windowProps.overflowScrollY)
          result.vertical = doneY = true;
        ...
        if (!doneX)
          result.horizontal =
            context.documentElement.clientWidth <
            $.documentWidth(context.document);
        if (!doneY)
          result.vertical =
            context.documentElement.clientHeight <
            $.documentHeight(context.document);
      }
    ...
    function _getViewportOverflows(query, context) {
      ...
      windowProps = getCss(context.documentElement, overflowPropNames, {
        toLowerCase: true,
      });
      bodyProps = getCss(context.body, bodyOverflowPropNames, {
        toLowerCase: true,
      });
    ...
    function getAppliedViewportOverflows(documentElementProps, bodyProps) {
      ...
      if (consolidated.window.overflowX === "visible")
        consolidated.window.overflowX = "auto";
      if (consolidated.window.overflowY === "visible")
        consolidated.window.overflowY = "auto";

      // In iOS, window.overflow(X/Y): "hidden" actually means "auto"; transform
      if (isIOS()) {
        if (consolidated.window.overflowX === "hidden")
          consolidated.window.overflowX = "auto";
        if (consolidated.window.overflowY === "hidden")
          consolidated.window.overflowY = "auto";
      }
    ...
    function wrapContainer(container) {
      ...
      if (
        ...
        throw new Error(
          "Invalid container: not a window, node, jQuery object or selector string",
        );
      ...
      if (!$container.length)
        throw new Error("Invalid container: empty jQuery object");
      ...
      if (container.nodeType === 9) {
        ...
        $container = wrapContainer(
          container.defaultView || container.parentWindow,
        );
      } else if (
    ...
    function checkHierarchy(elem, container) {
      ...
      if (elem.nodeType !== 1)
        throw new Error("Invalid node: is not an element");
      ...
      if (!elemIsContained)
        throw new Error("Invalid container: is not an ancestor of the element");
    }
    ...
    function checkOptions(opts) {
      ...
      if (
        ...
        throw new Error(
          'Invalid option value: direction = "' + opts.direction + '"',
        );
      }

      if (
        opts.box &&
        !(opts.box === "border-box" || opts.box === "content-box")
      ) {
      ...
      if (opts.tolerance !== undefined) {
        ...
        isNumWithUnit =
          isString(opts.tolerance) &&
          /^[+-]?\d*\.?\d+(px|%)?$/.test(opts.tolerance);
        if (!(isNum || isNumWithUnit))
          throw new Error(
            'Invalid option value: tolerance = "' + opts.tolerance + '"',
          );
      }
    }
      ...
      if (cache && cache.netContainerWidth !== undefined) {
        ...
        width = isWindow
          ...
          : $container.innerWidth() -
            getContainerScrollbarWidths($container, cache).vertical;
      ...
      if (cache && cache.netContainerHeight !== undefined) {
        ...
        height = isWindow
          ...
          : $container.innerHeight() -
            getContainerScrollbarWidths($container, cache).horizontal;
    ...
    function getCss(elem, properties, opts) {
      var i,
        ...
        _window =
          elem.ownerDocument.defaultView || elem.ownerDocument.parentWindow,
        computedStyles = _useGetComputedStyle
          ? _window.getComputedStyle(elem, null)
          : elem.currentStyle;
      ...
      for (i = 0; i < length; i++) {
        ...
        if (opts.toLowerCase && props[name] && props[name].toLowerCase)
