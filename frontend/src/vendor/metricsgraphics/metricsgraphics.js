;(function (root, factory) {
  ...
  function mg_data_in_plot_bounds (datum, args) {
    return (
      datum[args.x_accessor] >= args.processed.min_x &&
  ...
  function mg_is_horizontally_overlapping (element, labels) {
    ...
    for (var i = 0; i < labels.length; i++) {
      ...
      if (
        element_bbox.top === sibling_bbox.top &&
...
*/
  function warn_deprecation (message, untilVersion) {
    console.warn(
      'Deprecation: ' +
        message +
        (untilVersion ? '. This feature will be removed in ' + untilVersion + '.' : ' the near future.')
    )
    console.trace()
  }
...
*/
  ...
  function MGScale (args) {
    ...
    this.categoricalDomainFromData = function () {
      ...
      scaleArgs.categoricalVariables = d3
        .set(
          all_data.map(function (d) {
    ...
    this.categoricalRangeBands = function (range, halfway) {
      ...
      mg_add_scale_function(
        ...
        halfway ? args.scales[scaleArgs.scale_name].bandwidth() / 2 : 0
      )
    ...
    this.categoricalColorRange = function () {
      args.scales[scaleArgs.scale_name] =
        args.scales[scaleArgs.scale_name].domain().length > 10
  ...
  function addNumericalLabels (g, args, axisArgs) {
    ...
    if (ns == 'x') {
      if (args.time_series && args.european_clock) {
        labels
          .append('tspan')
          .classed('mg-european-hours', true)
          .text(function (_d, i) {
            var d = new Date(_d)
        ...
        labels
          .append('tspan')
          .classed('mg-european-minutes-seconds', true)
          .text(function (_d, i) {
            var d = new Date(_d)
  ...
  function categoricalLabelPlacement (args, axisArgs, group) {
    ...
    if (position === 'bottom') {
      ...
      coords.cat.textAnchor = args['rotate_' + ns + '_labels'] ? 'start' : 'middle'
      coords.group.x =
        groupScale(group) + (groupScale.bandwidth ? groupScale.bandwidth() / 2 - scale.bandwidth() / 2 : 0)
  ...
  function categoricalGuides (args, axisArgs) {
    ...
    grs.forEach(function (group) {
      ...
      var first =
        ...
        (scale.bandwidth() / 2) * (group === null || (position !== 'top' && position != 'bottom'))
      var last =
        groupScale(group) +
        scale(scale.domain()[scale.domain().length - 1]) +
        (scale.bandwidth() / 2) * (group === null || (position !== 'top' && position != 'bottom'))
    ...
    if (!yax_format) {
      ...
      if (args.format === 'count') {
        ...
        if (args.processed.y_ticks.length > 1) {
          ...
          decimals = Math.max(
            0,
            -Math.floor(Math.log(Math.abs(args.processed.y_ticks[1] - args.processed.y_ticks[0])) / Math.LN10)
          )
        }
  ...
  function x_axis_categorical (args) {
    ...
    mg_add_categorical_scale(
      args,
      'X',
      args.categorical_variables.reverse(),
      args.left,
      mg_get_plot_right(args) - additional_buffer
    )
  ...
  function mg_add_x_axis_categorical_labels (g, args, additional_buffer) {
    ...
    labels
      .attr('x', function (d) {
        return (
          args.scales.X(d) +
          args.scales.X.bandwidth() / 2 +
          args.buffer * args.bar_outer_padding_percentage +
          additional_buffer / 2
        )
      })
  ...
  function mg_remove_svg_if_chart_type_has_changed (svg, args) {
    if (
      (!svg.selectAll('.mg-main-line').empty() && args.chart_type !== 'line') ||
  ...
  function mg_categorical_count_number_of_lanes (args, ns) {
    ...
    if (groupAccessor) {
      ...
      group_bars = d3.max(
        Object.keys(group_bars).map(function (d) {
          return group_bars[d]
        })
      )
  ...
  function mg_categorical_calculate_height (args, ns) {
    ...
    var marginContribution =
      ns === 'y' ? args.top + args.bottom + args.buffer * 2 : args.left + args.right + args.buffer * 2

    return (
      groupContribution +
      marginContribution +
      args.categorical_groups.length *
        args[ns + 'group_height'] *
        (args[ns + 'group_padding_percentage'] + args[ns + 'group_outer_padding_percentage'])
    )
  }
  ...
  function mg_clear_mouseover_container (svg) {
    svg
      .selectAll('.mg-active-datapoint-container')
      .selectAll('*')
      .remove()
  }
...

  {
    ...
    var add_event_handler_for_brush = function add_event_handler_for_brush (args, target, axis) {
      ...
      var calculateSelectionRange = function calculateSelectionRange () {
        ...
        var range = {}
        range.x = axis.x
          ? [Math.max(min_x, Math.min(origin[0], mouse[0])), Math.min(max_x, Math.max(origin[0], mouse[0]))]
          : [min_x, max_x]
        range.y = axis.y
          ? [Math.max(min_y, Math.min(origin[1], mouse[1])), Math.min(max_y, Math.max(origin[1], mouse[1]))]
          : [min_y, max_y]
        return range
      }
    ...
    this.display = function () {
      ...
      for (var feature in this.feature_set) {
        ...
        $(this.target + ' div.segments').append(
          '<div class="btn-group ' +
          this._strip_punctuation(feature) +
          '-btns text-left">' + // This never changes.
          '<button type="button" class="btn btn-default btn-lg dropdown-toggle" data-toggle="dropdown">' +
          "<span class='which-button'>" +
          (this.public_name.hasOwnProperty(feature) ? this.public_name[feature] : feature) +
          '</span>' +
          "<span class='title'>" +
          (this.manual_callback.hasOwnProperty(feature) ? this.feature_set[feature][0] : 'all') +
          '</span>' + // if a manual button, don't default to all in label.
...

  {
    var mg_line_color_text = function mg_line_color_text (elem, _ref, _ref2) {
      ...
      elem
        .classed('mg-hover-line-color', color === null)
    ...
    var mg_add_area_generator = function mg_add_area_generator (_ref3, plot) {
      ...
      var areaBaselineValue = Number.isFinite(flip_area_under_y_value)
    ...
    var mg_add_line_element = function mg_add_line_element (_ref9, plot, this_path, which_line) {
      ...
      if (animate_on_load) {
        ...
        this_path
          .attr('d', plot.flat_line(data[which_line]))
          .transition()
          .duration(1000)
    ...
    var mg_add_legend_element = function mg_add_legend_element (args, plot, which_line, line_id) {
      ...
      if (args.legend) {
        ...
        if (args.legend_target) {
          ...
          var legend_text = plot.legend_group
            .append('svg:text')
    ...
    var mg_add_voronoi_rollover = function mg_add_voronoi_rollover (
      args,
      svg,
      rollover_on,
      rollover_off,
    ...
    var mg_add_aggregate_rollover = function mg_add_aggregate_rollover (
      args,
      svg,
      rollover_on,
      rollover_off,
    ...
    var mg_add_single_line_rollover = function mg_add_single_line_rollover (
      args,
      svg,
      rollover_on,
      rollover_off,
    ...
    var mg_line_main_plot = function mg_line_main_plot (args) {
      ...
      plot.display_area =
        (args.area && !args.use_data_y_min && args.data.length <= 1 && args.aggregate_rollover === false) ||
        (Array.isArray(args.area) && args.area.length > 0)
      plot.legend_text = ''
    ...
    var mg_line_rollover_setup = function mg_line_rollover_setup (args, graph) {
      ...
      if (mg_is_standard_multiline(args)) {
        mg_add_voronoi_rollover(
          args,
          svg,
          graph.rolloverOn(args),
          graph.rolloverOff(args),
          graph.rolloverMove(args),
          graph.rolloverClick(args)
        )
    ...
    var mg_update_aggregate_rollover_circle = function mg_update_aggregate_rollover_circle (_ref29, svg, datum) {
      ...
      svg
        .select('circle.mg-line-rollover-circle.mg-line' + datum.line_id)
    ...
    var mg_update_generic_rollover_circle = function mg_update_generic_rollover_circle (_ref30, svg, d) {
      ...
      svg
        .selectAll('circle.mg-line-rollover-circle.mg-line' + d.line_id)
    ...
    var mg_remove_active_data_points_for_aggregate_rollover = function mg_remove_active_data_points_for_aggregate_rollover (
      args,
      svg
    ) {
      svg
    ...
    var mg_remove_active_data_points_for_generic_rollover = function mg_remove_active_data_points_for_generic_rollover (
      _ref33,
      svg,
      _ref34
    ) {
    ...
    var histogram = function histogram (args) {
      ...
      this.mainPlot = function () {
        ...
        var bar = g
          ...
          .attr('transform', function (d) {
            return (
              'translate(' +
              args.scales.X(d[args.x_accessor]).toFixed(2) +
              ',' +
      ...
      this.rollover = function () {
        ...
        bar
          ...
          .attr('opacity', 0)
          .on('mouseover', _this.rolloverOn(args))
          .on('mouseout', _this.rolloverOff(args))
          .on('mousemove', _this.rolloverMove(args))
  ...
  function point_mouseover (args, svg, d) {
    ...
    if (args.color_accessor !== null && args.color_type === 'category') {
      ...
      row
        .text(label + '  ')
        .bold()
        .attr('fill', args.scalefns.colorf(d))
    }
...

  {
    var mg_filter_out_plot_bounds = function mg_filter_out_plot_bounds (data, args) {
      ...
      var new_data = data.filter(function (d) {
        return (
          (args.min_x === null || d[x] >= args.min_x) &&
...

  {
    ...
    var mg_targeted_legend = function mg_targeted_legend (_ref37) {
      ...
      if (legend_target) {
        var div = d3
          .select(legend_target)
          .append('div')
          .classed('mg-bar-target-legend', true)
    ...
    var legend_on_graph = function legend_on_graph (svg, args) {
      ...
      labels.forEach(function (label) {
        var sub_container = textContainer
          .append('tspan')
          .attr('x', mg_get_plot_right(args))
    ...
    var barChart = function barChart (args) {
      ...
      this.mainPlot = function () {
        ...
        if (args.reference_accessor !== null) {
          ...
          var reference_bars = barplot
            .selectAll('.mg-categorical-reference')
            .data(reference_data)
            .enter()
            .append('rect')
        ...
        if (args.comparison_accessor !== null) {
          ...
          var comparison_marks = barplot
            .selectAll('.mg-categorical-comparison')
      ...
      this.rollover = function () {
        ...
        bars
          .on('mouseover', _this3.rolloverOn(args))
          .on('mouseout', _this3.rolloverOff(args))
          .on('mousemove', _this3.rolloverMove(args))
        ...
        return function (d, i) {
          ...
          if (args.show_rollover_text) {
            ...
            if (args.predictor_accessor || args.baseline_accessor) {
              ...
              if (args.predictor_accessor)
                row.text(mg_format_data_for_mouseover(args, d, null, args.predictor_accessor, false))
              if (args.baseline_accessor)
                row.text(mg_format_data_for_mouseover(args, d, null, args.baseline_accessor, false))
            }
          }
          if (args.mouseover) {
            args.mouseover(d, i)
    ...
    this._format_element = function (element, value, args) {
      this.formatting_options.forEach(function (fo) {
        ...
        if (args[key])
          element.style(
            attr,
            typeof args[key] === 'string' || typeof args[key] === 'number' ? args[key] : args[key](value)
          )
      })
    }
  ...
  function process_line (args) {
    ...
    if ((args.missing_is_zero || args.missing_is_hidden) && args.chart_type === 'line' && is_time_series) {
      for (var i = 0; i < args.data.length; i++) {
        ...
        if (
          ['four-days', 'many-days', 'many-months', 'years', 'default'].indexOf(time_frame) !== -1 &&
