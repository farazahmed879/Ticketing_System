...
;(function (root, factory) {
  ...
  var bubbles = [
    ...
    {
      type: 'days',
      targets: [{ type: 'weeks', value: 7 }, { type: 'months', value: 31 }, { type: 'years', value: 365 }]
    },
    {
      type: 'months',
  ...
  function formatNumber (number, options, userLocale) {
    ...
    if (
      (maximumSignificantDigits && digitCount === maximumSignificantDigits + 1) ||
  ...
  function durationFormat () {
    ...
    var formatValue = function (momentType, index) {
      ...
      if (
        ...
        (momentType.significantDigits && momentType.significantDigits - momentType.wholeValue.toString().length <= 0)
      ) {
        ...
        if (useSignificantDigits && momentType.wholeValue) {
          ...
          momentType.value = Math.round(
            truncate(momentType.wholeValue, momentType.significantDigits - momentType.wholeValue.toString().length)
          )
    ...
    if (outputTypes && !(isMaxValue && !settings.trim)) {
      ...
      if (
        ...
        !(!trunc && momentTypes[0].isSmallest && momentTypes[0].rawValue < minValue)
      ) {
        momentTypes = []
      }
    }
    ...
    tokens = map(tokens, function (token) {
      ...
      if (
        isNegative &&
        (momentType.value > 0 || trim === '' || find(stopTrim, momentType.type) || find(outputTypes, momentType.type))
      ) {
        out += '-'
        isNegative = false
      }
