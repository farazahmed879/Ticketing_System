...
const Messages: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('chat:receive', (data: { roomId: string, message: Message }) => {
      ...
      setConversations(prev => {
        ...
        const updated = [...prev];
        updated[index] = { ...updated[index], recentMessage: data.message.body, updatedAt: new Date().toISOString() };
        // Sort by updatedAt
  ...
  useEffect(() => {
    if (activeConv) {
      const fetchMessages = async () => {
        try {
          const res = await api.get(API_ROUTES.MESSAGES.CONVERSATION_BY_ID(activeConv));
  ...
  const startChatWithUser = async (partnerId: string) => {
    try {
      const res = await api.post(API_ROUTES.MESSAGES.CONVERSATIONS, { partnerId });
      ...
      setConversations(prev => {
        ...
        const conv: Conversation = { id: newRoom.id, isGroup: false, partner, recentMessage: 'New Conversation', updatedAt: newRoom.updatedAt };
        return [conv, ...prev];
      });
  ...
  const createGroupChat = async () => {
    ...
    try {
      const res = await api.post(API_ROUTES.MESSAGES.GROUPS, { name: groupName, memberIds: selectedGroupMembers });
      const newRoom = res.data.conversation;
  ...
  const isStaff = user?.role?.isAdmin || user?.role?.isAgent ||
    user?.role?.name?.toLowerCase() === 'admin' || user?.role?.name?.toLowerCase() === 'agent';
  const isCustomer = user?.role?.isCustomer || user?.role?.name?.toLowerCase() === 'customer';
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {isStaff && (
              <button
                ...
                style={{ padding: 6, borderRadius: 8, cursor: 'pointer', color: 'var(--text-primary)' }}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontWeight: 600, fontSize: '0.95rem' }}>{conv.isGroup ? conv.name : conv.partner?.fullname}</span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{format(new Date(conv.updatedAt), 'HH:mm')}</span>
                  </div>
        ...
        {activeConv ? (
          <>
            <header className={styles.chatHeader}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600 }}>{selectedConv?.isGroup ? selectedConv.name : selectedConv?.partner?.fullname}</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                  {selectedConv?.isGroup ? `${selectedConv.members?.length || 0} members` : 'Active'}
                </div>
              </div>
            ...
            <div className={styles.messagesWrapper}>
              ...
              >
                {messages.map((msg) => (
                  <div key={msg.id} className={`${styles.messageWrapper} ${msg.senderId === user?.id ? styles.messageOwn : styles.messageOther}`}>
                    <div className={styles.messageBubble}>
                      {msg.body}
                    </div>
                    <span className={styles.messageTime}>{format(new Date(msg.createdAt), 'HH:mm')}</span>
                  </div>
                ))}
            ...
            <div className={styles.chatInput}>
              <form onSubmit={handleSendMessage} className="glass-card" style={{ display: 'flex', alignItems: 'center', padding: '8px 16px', gap: 12 }}>
                <input 
                  type="text" 
