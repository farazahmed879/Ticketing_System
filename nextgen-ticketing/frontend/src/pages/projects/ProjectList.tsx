...
const ProjectList: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      showNotification("error", err.response?.data?.error || "Failed to delete project");
    } finally {
      setIsLoading(false, "");
    }
  };
  ...
  const handleSubmit = async (data: any) => {
    setIsLoading(true, editingProject ? "Updating project..." : "Creating project...");
    try {
      if (editingProject) {
  ...
  const columns: TableColumn<Project>[] = [
    {
      ...
      render: (p) => (
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div className="glass-card" style={{ width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(16, 185, 129, 0.1)' }}>
            <CustomIcon name="FolderKanban" size={18} color="var(--accent-success)" />
          </div>
          <div>
            <div style={{ fontWeight: 600 }}>{p.name}</div>
            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>{p.description || "Company project"}</div>
          </div>
        </div>
      ),
    },
    {
      ...
      key: "department",
      render: (p) => (
        <span style={{ fontSize: "0.9rem" }}>{p.department?.name || "N/A"}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (p) => (
        <div style={{ display: "flex", gap: 8 }}>
          <CustomButton variant="ghost" size="sm" onClick={() => handleEdit(p)} icon={<CustomIcon name="Edit2" size={16} />} />
          <CustomButton variant="ghost" size="sm" onClick={() => handleDelete(p.id)} icon={<CustomIcon name="Trash2" size={16} />} style={{ color: "var(--accent-danger)" }} />
        </div>
      ),
    },
  ];
    ...
    <div className="animate-fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "var(--text-muted)" }}>Manage client and internal projects</p>
        </div>
        <CustomButton
          variant="gradient"
