...
const CommentNotePartial = props => {
  const { ticketSubject, comment, isNote, dateFormat, onEditClick, onRemoveClick, ticketStatus } = props
  const bodyRef = React.useRef(null)

  React.useEffect(() => {
    if (bodyRef.current) {
      const imagesEl = bodyRef.current.querySelectorAll('img:not(.hasLinked)')
      imagesEl.forEach(i => helpers.setupImageLink(i))
      const linksEl = bodyRef.current.querySelectorAll('a')
      linksEl.forEach(i => helpers.setupLinkWarning(i))
    }
  }, [comment])
